    ...
    while (!feof(f)) {
        ...
        if (in_global) {
            ...
            if (pa_streq(v, ATTACHED_OUTPUT_DEVICES_TAG))
                ...
                success = true; // Do not fail while parsing speaker_drc_enabled tag
            else {
...
static void add_o_ports(pa_droid_mapping *am) {
    ...
    while (devices) {
        ...
        if (devices & cur_device) {
            ...
            pa_log("device is 0x%x and devices is 0x%x", cur_device, devices);
    ...
    if (am->output->devices & combo_devices) {
        pa_log("device is 0x%x", combo_devices);
